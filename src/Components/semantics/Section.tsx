...
function Section({ framer, children, className, style, framerProps, attributes, key }: SemanticProps) {
    type SectionMotionProps = typeof motion.section extends React.FC<infer P>
  ? P
  : never;
  return (
    <>
     {framer ? (
      <motion.section {...(framerProps as SectionMotionProps)} className={cn('', className)} style={style} key={key}>
      </motion.section>
     ) : (
